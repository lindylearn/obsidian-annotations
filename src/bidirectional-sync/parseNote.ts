...
const getFileSection = (text: string, header: string): string => {
    ...
    return text.slice(sectionStart, sectionEnd).replace(header, '').trim();
};
...
const parseAnnotationText = (text: string): LocalHighlight => {
    ...
    for (const line of text.split('\n')) {
        if (line.startsWith('>')) {
            ...
            const match =
                /https\:\/\/hyp.is\/(?<threadId>[^\/]+)\/(?<url>.*)#(?<annotationId>[^\/ )]+)/g.exec(
                    line
                );
            if (!match) {
                console.error(
                    `Found annotation without valid id link: ${text}`
                );
                return null;
            }
    ...
    if (noteBulletPoints[noteBulletPoints.length - 1]?.startsWith('#')) {
        let tagsText = noteBulletPoints.pop().trim();
